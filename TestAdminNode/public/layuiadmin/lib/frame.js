...
//		$("#frameMainTitle span").html('<i class="iconfont icon-xianshiqi"></i>'+$(_this).parent().parent().siblings(".menuFA").text());
	...
	//处理菜单样式变化
	$("a").css("cssText", "background-color:#transparent")
	$(_this).css("cssText", "background-color:#009688 !important;")
	$(".layui-this").removeClass("layui-this")
	$(_this).parent().siblings().find("a").css("cssText", "background-color:#transparent")
//	$(_this).parent().parent().parent().siblings().find("dl dt a").css("cssText", "background-color:#transparent").css("color","white")
//	$(_this).parent().parent().parent().siblings().find("dl dd a").css("cssText", "background-color:#transparent").css("color","white")
}
